...
export class MeasurementService {
  ...
  getMeasurementByDevIdAndType(deviceId: number, type: string, peagable: number) {
    return this.http.post<MeasurementDTO[]>(
      this.apiUrl + "/measurement/getMeasurementByDevIdAndType",
      null,
      {params: {deviceId, type, peagable}})


  }

  getMeasurementByFields(devicesId: number | 0, fieldKey: string) {
    return this.http.post<Measurement>(this.apiUrl + "/measurement/getMeasurementByField", null, {
      params: {
        devicesId,
        fieldKey
      }
